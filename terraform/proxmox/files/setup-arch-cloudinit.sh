...
then
	...
	qm create "${VMID}" -name "${VMNAME}" -memory 1024 -net0 virtio,bridge=vmbr0 -cores 1 -sockets 1 -cpu cputype=kvm64 -description "arch image" -kvm 1 -numa 1
	qm importdisk "${VMID}" arch-openstack-amd64.qcow2 local-zfs
